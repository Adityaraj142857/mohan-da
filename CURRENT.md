...
1. **Business Operations Platform Extension**:
   - `shopbot.analytics` package (pure DB aggregation, deterministic logic, no LLMs).
   - Sales KPIs, peak ordering hours, and top products.
   - Customer intelligence (segmentation, typical ordering hour, favorite items).
   - Cross-selling engine (historical co-occurrence / association analysis).
   - Offer recommendations (velocity-drop detection).
   - Inventory tracking (weighted moving average demand estimation, restock alerts).
   - Daily business summary generator (plain text).
   - **Admin UI Redesign**: The previous `/admin` kanban board moved to `/admin/board`. The new landing page is a comprehensive dashboard. Added `/admin/inventory`, `/admin/recommendations`, and `/admin/customers/{id}` pages.
   - **Demo Data**: Added `python -m shopbot seed-demo-analytics` for a rich demo environment.
   - **Testing**: Added 11 new tests (`tests/unit/test_analytics_*.py`), bringing the total suite to 141 tests.

2. **Rubric-weighted improvements to the assignment deliverables:**
